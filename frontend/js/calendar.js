...
async function loadHolidaysForRange(start, end) {
    ...
    try {
        ...
        await Promise.all(yearsToLoad.map(async (year) => {
            const data = await fetchAPI(`/calendar/holidays?year=${year}`);
...
async function loadEvents() {
    try {
        events = await fetchAPI('/calendar');
        if (calendar) {
...
async function handleSubmit(event) {
    ...
    try {
        if (editingEventId) {
            // 🔥 수정 모드일 때
            await fetchAPI(`/calendar/${editingEventId}`, {
                method: 'PUT',
...
async function handleEventListClick(event) {
    ...
    if (deleteBtn) {
        ...
        try {
            await fetchAPI(`/calendar/${deleteBtn.dataset.eventId}`, { method: 'DELETE' });
