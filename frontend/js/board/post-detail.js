...
function setupActionButtons(post, user, boardCode, postId, returnPath) {
    ...
    const boardRoutes = getBoardRouteConfig(boardCode);
    const detailPath = boardRoutes.detail(postId);

    // SPA 환경에서 중복 이벤트 방지를 위해 onclick을 사용
    editBtn.onclick = () => {
        navigateTo(`${boardRoutes.edit(postId)}?board=${encodeURIComponent(boardCode)}&return=${encodeURIComponent(detailPath)}`);
    };
    ...
    deleteBtn.onclick = async () => {
        ...
        setBusy(true);
        try {
            await deletePost(boardCode, postId);
            alert('게시글이 삭제되었습니다.');
