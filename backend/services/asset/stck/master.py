...
from repositories.asset.stck import master as master_repository
...
def get_my_stock_master_or_404(master_key: str, db: Session, user_id: int) -> StockMaster:
    proc_date_value, item_code = keys.parse_master_key(master_key)
...
def build_stock_master_rows(db: Session, user_id: int, proc_date_value: date) -> list[StockMaster]:
    ...
    for item_code in sorted(item_codes):
        ...
        if current_stcn < 0:
            raise bad_request(f"{item_code} 보유수량이 음수입니다. 전일 보유 또는 매도 수량을 확인해주세요.")
...
def list_my_stock_masters(
    ...
    return [responses.build_master_response(master, item_map.get(master.itms_code)) for master in masters]
...
def generate_my_stock_masters(
    ...
    return [responses.build_master_response(master, item_map.get(master.itms_code)) for master in generated]
...
def create_my_stock_master(
    ...
    return responses.build_master_response(master, stock_item)
