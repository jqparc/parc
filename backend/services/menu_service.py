...
class MenuService:
    ...
    # 👉 현재 로그인한 유저의 user_id를 기반으로 메뉴 리스트를 가져오는 함수
    def get_menus_for_role(self, role: str):
        menus = self.menu_repo.get_list_by_role(role)
        
        # 2. 조회된 메뉴가 없다면 (빈 리스트 반환 시)
        if not menus:
            # 3. 공통(COMMON) 메뉴 설정값을 가져옵니다.
            menus = self.menu_repo.get_list_by_role("ALL")
