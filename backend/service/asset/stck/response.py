...
from model.asset import StockItem, StockMaster, StockTrade
from schema.asset.stck import StockItemResponse, StockItemSearchResponse, StockMasterResponse, StockTradeResponse
from service.asset.stck import key
from service.asset.stck.market_data import resolve_current_price


def build_stock_item_response(item: StockItem) -> StockItemResponse:
    data = {
        "item_key": key.make_item_key(item.proc_date, item.itms_code),
...
def build_master_response(master: StockMaster, stock_item: StockItem | None = None) -> StockMasterResponse:
    ...
    data = {
        ...
        "master_key": key.make_master_key(master.proc_date, master.itms_code),
...
def build_trade_response(trade: StockTrade, stock_item: StockItem | None = None) -> StockTradeResponse:
    ...
    data = {
        ...
        "trade_key": key.make_trade_key(trade.proc_date, trade.itms_code, trade.trns_code, trade.seq),
