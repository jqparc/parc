...
from model.asset import StockItem
...
from service.asset.stck.market_data import resolve_stock_item_snapshot


def get_stock_item_or_404(item_key: str, db: Session) -> StockItem:
    proc_date_value, item_code = key.parse_item_key(item_key)
...
def apply_stock_item_data(item: StockItem, data: dict) -> None:
    ...
    item.bzty_code = key.normalize_bzty_code(data.get("bzty_code"))
    item.clpr = data.get("clpr")


def list_stock_items(db: Session) -> list[StockItemResponse]:
    return [response.build_stock_item_response(item) for item in item_repository.get_stock_items(db)]
...
def search_stock_items(
    ...
    return [
        response.build_stock_item_search_response(item, shtg_name, bzty_name)
...
def create_stock_item(db: Session, item_data: StockItemCreate) -> StockItemResponse:
    ...
    return response.build_stock_item_response(item)
...
def generate_my_stock_items(
    ...
    for item in generated:
        unit_of_work.refresh(db, item)
    return [response.build_stock_item_response(item) for item in generated]


def get_stock_item_detail(db: Session, item_key: str) -> StockItemResponse:
    return response.build_stock_item_response(get_stock_item_or_404(item_key, db))
