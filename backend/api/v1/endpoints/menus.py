...
from schemas.menu_schema import MenuResponse
...
def read_menu_me(
    ...
    service: MenuService = Depends(get_menu_service) # 메뉴 서비스 주입
):
    print("메뉴 목록 조회 요청:", current_user.role.value)
    
    # 서비스 계층에 유저 ID를 넘겨 메뉴 목록을 받아옵니다.
    menus = service.get_menus_for_role(current_user.role.value)
